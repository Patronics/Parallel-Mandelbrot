...
__global__ void compute_image(coordSet* coords, int width, int height, struct colors *colorsSet, int blockCount, int blockSize)
{
...
void reDraw(coordSet* coords){
	...
	compute_image <<<blockCount, blockSize>>>(cudaCoords, width, height, colorsSet, blockCount, blockSize);
