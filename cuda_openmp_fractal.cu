...
void compute_imageCPU(coordSet* coords, int width, int height, struct colors *colorsSet)
{
        double xmin=coords->xmin;
        double xmax=coords->xmax;
        double ymin=coords->ymin;
...
void reDraw(coordSet* coords){
	...
	dim3 dimGrid(width/BLOCK_SIZE, height/BLOCK_SIZE*0.6); // 1*1 blocks in a grid
	...
	compute_imageCPU(coords, width, height, c);
	err = cudaDeviceSynchronize();
	if (err != cudaSuccess) printf("%s synch\n", cudaGetErrorString(err));
	err = cudaMemcpy(c, colorsSet, n*0.6 * sizeof(struct colors), cudaMemcpyDeviceToHost);
	...
	for (int i = 0; i < height; i++)
		for (int j = 0; j < width; j++){
			//c[i * width + j].r=j;
			        gfx_color(c[i*width+j].r, c[i*width+j].g, c[i*width+j].b);
        			// Plot the point on the screen.
        			gfx_point(j, i);
		}
...
int main( int argc, char *argv[] ){
	...
	// Open a new window.
	gfx_open(1280,960,"Mandelbrot Fractal");
